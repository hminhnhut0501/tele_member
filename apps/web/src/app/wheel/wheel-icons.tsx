...
export function FixedWheelIcon({ kind, size = 44, variant = 1 }: { kind: WheelIconKind; size?: number; variant?: 1 | 2 }) {
      ...
      <svg {...sharedProps} width={size} height={size}>
        <defs>
          <linearGradient id="wheel-peach-body" x1="14" y1="12" x2="34" y2="40" gradientUnits="userSpaceOnUse">
            <stop stopColor={alternate ? '#FFD1F0' : '#FFD39B'} />
            <stop offset="0.42" stopColor={alternate ? '#F27AC8' : '#FF996B'} />
            <stop offset="1" stopColor={alternate ? '#C53F91' : '#EF5B4D'} />
          </linearGradient>
          <linearGradient id="wheel-peach-leaf" x1="30" y1="6" x2="39" y2="16" gradientUnits="userSpaceOnUse">
            <stop stopColor={alternate ? '#C4B5FD' : '#B8F27D'} />
            <stop offset="1" stopColor={alternate ? '#6D50C8' : '#42A85F'} />
          </linearGradient>
        </defs>
        ...
        <path d="M25 12C27.7 5.8 34.1 5.2 39 8.5C37.2 14.6 32.3 17 25 14.2" fill="url(#wheel-peach-leaf)" stroke={alternate ? '#4C358D' : '#2F7E4A'} strokeWidth="1.4" />
  ...
  return (
    <svg {...sharedProps} width={size} height={size}>
      ...
      <path d="M24 4L28.2 19.8L44 24L28.2 28.2L24 44L19.8 28.2L4 24L19.8 19.8L24 4Z" fill="url(#wheel-sparkle)" stroke={alternate ? '#B87926' : '#7EA9F7'} strokeWidth="1.4" strokeLinejoin="round" />
